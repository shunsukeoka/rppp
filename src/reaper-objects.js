...
const { EMPTY_RPP } = require('./empty-rpp')
...
class ReaperProject extends ReaperBase {
    ...
    constructor(obj) {
        if (obj) super(obj)
            ...
            const empty = parser.parse(EMPTY_RPP)
            super(empty)
        }
    }
...
>`,
    ...
    isWaveSource() {
        return this.params[0] === 'WAVE'
    }
    isMidiSource() {
        return this.params[0] === 'MIDI'
    }
    isMp3Source() {
        return this.params[0] === 'MP3'
    }
    isSectionSource() {
        return this.params[0] === 'SECTION'
    }
    ...
    makeSectionSource() {
        ...
        if (this.getStructByToken('SOURCE')) {
            ...
            // ever have children.
            console.warn(
                'WARNING: rppp called .makeSectionSource on an object that already has one or more SOURCE children. This may result in bugs',
            )
        }
